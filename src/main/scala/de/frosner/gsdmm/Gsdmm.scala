...
case class Gsdmm(documents: Array[List[String]],
                 numClusters: Int,
  ...
  private def sampleNewCluster(mz_d: mutable.HashMap[ClusterId, Count],
    ...
    if (debug) println(s"""  Document $documentId: ${pz.mkString(" ")}""")
    val pzSum = pz.sum
